...
import torch
import pandas as pd
from monai.transforms import (Compose, Lambdad, NormalizeIntensityd,RandFlipd,RandCoarseShuffled,RandRotated,RandZoomd,
...
class QaTa(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt = torch.mean(gt.float(), dim=0).unsqueeze(0)
        # gt = torch.where(gt == 255.0, 1, 0)
        # gt = torch.where(gt >= 127.5, 1, 0) # 改了这个提分了
        gt = torch.where(gt > 0, 1, 0) # 改了这个提分了
    ...
    def transform(self,image_size=[224,224]):
        ...
        if self.mode == 'train':  # for training mode
            trans = Compose([
                ...
                RandFlipd(['image','gt'],prob=0.5),
                RandFlipd(['image', 'gt'], prob=0.5, spatial_axis=1),#mosmed
                RandRotated(['image','gt'],prob=0.5,range_x=(0.0,360.0)),
                RandCoarseShuffled(['image'], holes=10, spatial_size=(16, 16), prob=0.2),#mosmed
